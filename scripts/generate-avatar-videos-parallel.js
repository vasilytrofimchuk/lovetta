#!/usr/bin/env node
/**
 * Generate videos for all custom avatars WITHOUT videos — all submitted in parallel.
 * Uses fal.ai async queue: submit all → poll all → collect results.
 */
...
async function submitJob(avatar) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`submit ${res.status}: ${err}`);
  }
  const data = await res.json();
  return { ...avatar, request_id: data.request_id, status_url: data.status_url, response_url: data.response_url };
}
...
async function pollResult(job) {
  ...
  while (Date.now() - start < maxWait) {
    ...
    if (status.status === 'COMPLETED') {
      ...
      if (!falUrl) throw new Error('no video url in result');
      // Upload to R2
...
async function main() {
  console.log(`Submitting ${AVATARS.length} jobs in parallel...\n`);

  // Phase 1: Submit ALL jobs
  const jobs = [];
  const submitResults = await Promise.allSettled(AVATARS.map(async (a, i) => {
    try {
      const job = await submitJob(a);
      console.log(`[${String(i+1).padStart(2)}] submitted: ${job.request_id}`);
      return job;
    } catch (err) {
      console.error(`[${String(i+1).padStart(2)}] submit FAILED: ${err.message}`);
      return null;
    }
  }));

  const submitted = submitResults.filter(r => r.status === 'fulfilled' && r.value).map(r => r.value);
  console.log(`\n${submitted.length}/${AVATARS.length} submitted. Polling for results...\n`);

  // Phase 2: Poll ALL results in parallel
  const results = await Promise.allSettled(submitted.map(async (job, i) => {
    try {
      const videoUrl = await pollResult(job);
      console.log(`[${job.id.slice(0,8)}] ✓ ${videoUrl}`);
      return { id: job.id, type: job.type, video: videoUrl };
    } catch (err) {
      console.error(`[${job.id.slice(0,8)}] ✗ ${err.message}`);
      return { id: job.id, type: job.type, video: null };
    }
  }));

  // Print mapping
  console.log('\n=== VIDEO MAPPING ===');
  const succeeded = results.filter(r => r.status === 'fulfilled' && r.value?.video).map(r => r.value);
  for (const r of succeeded) {
    console.log(`${r.id} → ${r.video}`);
  }
  console.log(`\nDone: ${succeeded.length}/${AVATARS.length} succeeded`);
}
